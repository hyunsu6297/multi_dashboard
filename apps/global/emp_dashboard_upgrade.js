(function () {
  ...
  const moveRowsUpButton = document.createElement("button");
  moveRowsUpButton.className = "actionBtn";
  moveRowsUpButton.id = "moveSelectedEmpRowsUp";
  moveRowsUpButton.textContent = "▲ 위로";
  const moveRowsDownButton = document.createElement("button");
  moveRowsDownButton.className = "actionBtn";
  moveRowsDownButton.id = "moveSelectedEmpRowsDown";
  moveRowsDownButton.textContent = "▼ 아래로";
  ...
  exportTradesButton.after(moveRowsUpButton);
  moveRowsUpButton.after(moveRowsDownButton);
  const saveEtfs = () => {
  ...
  const moveSelectedEmpRows = direction => {
    const rows = empRows();
    const selected = [...selectedRows].filter(index => index >= 0 && index < rows.length);
    if (!selected.length) {
      const status = document.getElementById("empStatus");
      if (status) status.textContent = "이동할 행을 체크하세요.";
      return;
    }
    const ordered = selected.sort((a, b) => direction < 0 ? a - b : b - a);
    const nextSelection = new Set(selectedRows);
    let moved = false;
    ordered.forEach(index => {
      const nextIndex = index + direction;
      if (nextIndex < 0 || nextIndex >= rows.length || nextSelection.has(nextIndex)) return;
    ...
    if (!moved) return;
    selectedRows.clear();
    [...nextSelection].sort((a, b) => a - b).forEach(index => selectedRows.add(index));
    markEmpDirty("행 순서 변경됨 · 변경저장을 눌러 확정");
    renderEmp();
  };
  ...
  renderEmp = function () {
    ...
    const colWidths = [2, 6, 6.5, 3, 3.5, 3.5, 3.5, 3.5, 4.5, 4.5, 4.8, 3.4, 5.2, 3.6, 5.4, 3.8, 5.2, 3.3, 3, 4, 5.6];
  ...
  document.getElementById("moveSelectedEmpRowsUp").onclick = () => moveSelectedEmpRows(-1);
  document.getElementById("moveSelectedEmpRowsDown").onclick = () => moveSelectedEmpRows(1);
