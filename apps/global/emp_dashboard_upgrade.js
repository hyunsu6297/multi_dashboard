(function () {
  ...
  style.textContent = `
    ...
    .dashboardPieColumn{display:grid;grid-template-rows:1fr 1fr;gap:10px;min-height:0}.piePanel{height:100%;min-height:0;display:flex;flex-direction:column}.pieBox{position:relative;flex:1;min-height:0;display:grid;place-items:center;overflow:visible}.pieChart{position:relative;width:min(351px,calc(100% + 78px));aspect-ratio:1;border-radius:50%;margin:auto;background:#e4efec;box-shadow:0 0 0 1px #d4e5e1}.pieLabel{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);max-width:82px;text-align:center;color:#fff;font-size:10px;font-weight:900;line-height:1.15;text-shadow:0 1px 3px rgba(0,0,0,.55);pointer-events:none}.pieCallout{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:2;min-width:54px;max-width:86px;text-align:center;color:#173a34;background:rgba(255,255,255,.92);border:1px solid rgba(96,157,145,.55);border-radius:8px;padding:3px 5px;font-size:10px;font-weight:900;line-height:1.15;box-shadow:0 2px 7px rgba(0,0,0,.08);pointer-events:none}.pieLeaders{position:absolute;inset:0;overflow:visible;pointer-events:none}.pieLeaders line{stroke:#6c7f7b;stroke-width:1.2;stroke-dasharray:2 2}.pieLegend{display:none}.unclassifiedBtn{border:1.5px solid #d4a72c;background:#fff7d7;color:#7c5510;font-weight:900;border-radius:5px;padding:9px 12px;cursor:pointer;white-space:nowrap}
    #dashboard .tradeLongPanel .tablewrap{height:auto!important;max-height:none!important;flex:1;min-height:0;overflow-x:hidden}#dashboard .tradeLongPanel table{font-size:10px;table-layout:fixed;width:100%}#tradeTable th,#tradeTable td{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:3px;padding-right:3px}#tradeTable th:nth-child(1),#tradeTable td:nth-child(1){width:15.5%;text-overflow:clip}#tradeTable th:nth-child(2),#tradeTable td:nth-child(2){width:18%;text-overflow:clip}#tradeTable th:nth-child(3),#tradeTable td:nth-child(3){width:6%}#tradeTable th:nth-child(4),#tradeTable td:nth-child(4){width:16%;max-width:0}#tradeTable th:nth-child(5),#tradeTable td:nth-child(5){width:9%}#tradeTable th:nth-child(6),#tradeTable td:nth-child(6){width:6%}#tradeTable th:nth-child(7),#tradeTable td:nth-child(7){width:9%}#tradeTable th:nth-child(8),#tradeTable td:nth-child(8){width:9%}#tradeTable th:nth-child(9),#tradeTable td:nth-child(9){width:11.5%}#tradeTable span[title]{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.tradeEmpty{height:100%;min-height:360px;display:grid;place-items:center;color:#6c7f7b;font-weight:900;border:1px dashed #b8d2cc;border-radius:10px;background:#f8fcfb}.sortHeader{width:100%;border:0;background:transparent;color:inherit;font:inherit;font-weight:900;cursor:pointer;text-align:center;padding:0}.sortHeader:hover{text-decoration:underline}
  ...
  const savedMarket = JSON.parse(localStorage.getItem("globalDashboard.market") || "null");
  ...
  editEmp = function (index, key, value) {
    ...
    if (key === "security") row[key] = value;
    else if (key === "quantity") {
      row.quantity = parseNumber(value);
      row.quantityTouched = true;
    }
    else if (key === "targetWeight") {
      if (String(value).trim() === "") { row.targetTouched = false; delete row.targetWeight; }
      else { row.targetTouched = true; row.targetWeight = parseNumber(value) / 100; }
  ...
  renderEmp = function () {
    ...
    document.querySelectorAll("#empTable input[data-i][data-key]").forEach(input => input.onchange = () => editEmp(Number(input.dataset.i), input.dataset.key, input.value));
  ...
  refreshMarket = async function () {
    ...
    try {
      const res = await fetch(marketApiUrl(), {
        ...
        body: JSON.stringify({ securities: [...new Set([...allRows.map(row => row.security).filter(Boolean), ...fundSecurities])] })
      });
